...
(()=>{
  ...
  async function bindFooterContacts(){const form=$('#appearanceForm');if(!form||form.dataset.footerContacts)return;form.dataset.footerContacts='true';const db=window.supabase.createClient(window.TINGS_SUPABASE.url,window.TINGS_SUPABASE.anonKey),{data}=await db.from('shop_settings').select('content').eq('id',1).maybeSingle(),saved=data?.content?.footerAppearance||{},config={...footerDefaults,...saved,socials:{...footerDefaults.socials,...(saved.socials||{})}};$('#footerShowPhone').checked=config.showPhone!==false;$('#footerShowEmail').checked=config.showEmail!==false;footerSocials.forEach(([id])=>{const social={...footerDefaults.socials[id],...(config.socials[id]||{})},upload=$(`#footerQr_${id}`),preview=$(`#footerQrPreview_${id}`);$(`#footerSocial_${id}`).checked=!!social.show;form.dataset[`footerQr_${id}`]=social.qr||'';preview.innerHTML=social.qr?`<img src="${social.qr}" alt="${id} 二维码">`:'未上传';upload.onchange=e=>{const file=e.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{form.dataset[`footerQr_${id}`]=reader.result;preview.innerHTML=`<img src="${reader.result}" alt="${id} 二维码">`;};reader.readAsDataURL(file);};});}
