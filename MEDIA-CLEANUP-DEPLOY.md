...
保护登记永久保留，包括 Storage 删除失败、超时或响应丢失的情况。已有登记不等于正在引用：仅当文件仍存在、同一事务锁内重新确认未被引用且通过默认路径保护，正常工作流再次通过年龄等删除资格检查后，才允许重试 Storage DELETE。已登记但文件已不存在时视为删除完成，不再获得删除资格。重试不修改退役时间、不解除保护，路径永久禁止复用，因此迟到或重叠的删除请求不能误删替换文件。受保护路径需要重新上传为全新路径，不得通过清空登记表恢复使用。回滚函数时不得回退到未使用登记保护的旧删除实现。

已应用原保护迁移的环境使用 `media-deletion-retry-migration.sql` 增量替换 RPC；未应用保护迁移的环境使用包含同一修复的完整 `media-deletion-guard-migration.sql`。生产应用须另行审批；本次测试项目证据见 `P1-REGRESSION.md`。
