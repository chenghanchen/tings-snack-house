...
declare
  ...
  campaign_discount numeric(10,2):=0; code_discount numeric(10,2):=0; total_discount numeric(10,2):=0;
  fee numeric(10,2); tax numeric(10,2); total numeric(10,2); store_tax numeric(6,3); free_at numeric(10,2); base_fee numeric(10,2);
...
begin
  ...
  for campaign in select * from public.marketing_campaigns where active=true and kind in ('full_reduction','product_discount','category_discount') and (starts_at is null or starts_at<=now()) and (ends_at is null or ends_at>=now()) order by created_at,id loop
    ...
    if campaign.kind='full_reduction' and item_subtotal>=campaign.threshold then
      ...
      if cardinality(campaign.product_ids)=0 and cardinality(campaign.category_names)=0 then eligible_subtotal:=item_subtotal;
      else
  ...
  if entered_code is not null and entered_code<>'' then
    ...
    if found then
      ...
      if item_subtotal<referral_min then raise exception '未达到推荐奖励最低消费金额'; end if;
      code_discount:=least(referral_amount,item_subtotal-campaign_discount); selected_code_name:='推荐码奖励'; selected_code_kind:='referral';
    else
      ...
      code_discount:=least(coupon.amount,item_subtotal-campaign_discount); selected_code_name:=coupon.name; selected_code_kind:='coupon'; selected_coupon_id:=coupon.id;
    end if;
  end if;
  total_discount:=greatest(campaign_discount+code_discount,0); taxable:=greatest(item_subtotal-total_discount,0); tax:=round(taxable*store_tax/100,2); total:=round(taxable+tax+fee,2);
  ...
  values(order_number,trim(p_customer_name),p_phone,nullif(trim(p_email),''),p_fulfillment,nullif(trim(p_address),''),nullif(trim(p_note),''),lines,item_subtotal,total_discount,selected_campaign_id,coalesce(selected_campaign_kind,selected_code_kind),concat_ws(' + ',selected_campaign_name,selected_code_name),entered_code,jsonb_build_object('campaign_name',selected_campaign_name,'campaign_kind',selected_campaign_kind,'campaign_discount',campaign_discount,'code_name',selected_code_name,'code_kind',selected_code_kind,'code_discount',code_discount),store_tax,tax,fee,total,'待确认',false) returning id into order_id;
  ...
  return jsonb_build_object('id',order_id,'order_number',order_number,'subtotal',item_subtotal,'discount_amount',total_discount,'promotion_name',concat_ws(' + ',selected_campaign_name,selected_code_name),'tax_rate',store_tax,'tax_amount',tax,'delivery_fee',fee,'total_amount',total);
end $$;
