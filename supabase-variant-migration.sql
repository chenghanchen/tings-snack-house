...
with ranked as (select id,row_number() over(order by id)-1 as new_position from public.categories)
update public.categories c set position=r.new_position from ranked r where c.id=r.id and c.position=0;
insert into public.categories(name,position,is_system)
select '未分类',coalesce((select max(position)+1 from public.categories),0),true
where not exists (select 1 from public.categories where name='未分类');
update public.categories set is_system=true where name='未分类';

create or replace function public.reject_orders_when_closed() returns trigger language plpgsql security definer set search_path=public as $$
begin
  if exists (select 1 from public.shop_settings where id=1 and is_accepting_orders=false) then
    raise exception '店铺暂不接单';
  end if;
  return new;
end $$;
drop trigger if exists reject_orders_when_closed on public.orders;
create trigger reject_orders_when_closed before insert on public.orders for each row execute function public.reject_orders_when_closed();
